    ...
    */
    ...
    public R save(@RequestBody YujingEntity yujing, HttpServletRequest request){
        ...
        Wrapper<YujingEntity> queryWrapper = new EntityWrapper<YujingEntity>()
            ...
            .eq("yujing_address", yujing.getYujingAddress())
...
//        if(false)
//            return R.error(511,"永远不会进入");
        if("".equals(yujing.getYujingFile()) || "null".equals(yujing.getYujingFile())){
                yujing.setYujingFile(null);
        }
        if("".equals(yujing.getYujingContent()) || "null".equals(yujing.getYujingContent())){
                yujing.setYujingContent(null);
        }
...
//                            yujingEntity.setYujingUuidNumber(data.get(0));                    //预警编号 要改的
//                            yujingEntity.setYujingName(data.get(0));                    //预警标题 要改的
//                            yujingEntity.setYujingAddress(data.get(0));                    //预警地点 要改的
//                            yujingEntity.setYujingFile(data.get(0));                    //附件 要改的
//                            yujingEntity.setYujingTypes(Integer.valueOf(data.get(0)));   //预警类型 要改的
//                            yujingEntity.setYujingTime(sdf.parse(data.get(0)));          //预警时间 要改的
...
//                            yujingEntity.setCreateTime(date);//时间
                        ...
                        if(yujingEntities_yujingUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [预警编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
    ...
    */
    ...
    public R add(@RequestBody YujingEntity yujing, HttpServletRequest request){
        ...
        Wrapper<YujingEntity> queryWrapper = new EntityWrapper<YujingEntity>()
            .eq("yujing_uuid_number", yujing.getYujingUuidNumber())
            .eq("yujing_name", yujing.getYujingName())
            .eq("yujing_address", yujing.getYujingAddress())
            .eq("yujing_types", yujing.getYujingTypes())
